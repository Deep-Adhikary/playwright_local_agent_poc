        ...
        if estimate_tokens(text) <= max_tokens_per_chunk:
            ...
            for j, child in enumerate(cr.children):
                ...
                if estimate_tokens(child_text) <= max_tokens_per_chunk:
                    chunks.append({"chunk_id": f"ch{idx}_{j}", "root_role": f"{cr.role}:{child.role}", "text": child_text})
                else:
                    # last resort: hard-slice text (still better than failing)
                    chunks.append({"chunk_id": f"ch{idx}_{j}_slice", "root_role": f"{cr.role}:{child.role}", "text": child_text[: max_tokens_per_chunk * 4]})
    return chunks
